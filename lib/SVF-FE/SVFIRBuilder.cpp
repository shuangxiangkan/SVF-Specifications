...
void SVFIRBuilder::initialiseNodes()
{
    ...
    for (SymbolTableInfo::ValueToIDMapTy::iterator iter =
             symTable->valSyms().begin();
         iter != symTable->valSyms().end();
         ++iter)
    {
    ...
    for (SymbolTableInfo::FunToIDMapTy::iterator iter =
             symTable->retSyms().begin();
         iter != symTable->retSyms().end();
         ++iter)
    {
    ...
    assert(pag->getTotalNodeNum() >= symTable->getTotalSymNum() && "not all node been inititalize!!!");
}
